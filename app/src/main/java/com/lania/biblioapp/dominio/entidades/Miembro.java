package com.lania.biblioapp.dominio.entidades;
...
public class Miembro {
    ...
    public static Miembro createMiembroByContentValues(ContentValues values) {
        String nombre = values.getAsString(_NOMBRE);
        String fechaDeNacimiento = values.getAsString(_FECHA_DE_NACIMIENTO);
        String direccion = values.getAsString(_DIRECCION);

        LocalDateTime fechaRegistro = null;
        if (values.containsKey(_FECHA_DE_REGISTRO)) {
            String fechaRegistroStr = values.getAsString(_FECHA_DE_REGISTRO);
            fechaRegistro = LocalDateTime.parse(fechaRegistroStr, DateConvertes.FORMATTER);
        }

        return createMiembro(0L, nombre, LocalDateTime.parse(fechaDeNacimiento, DateConvertes.FORMATTER), direccion, fechaRegistro);
    }
